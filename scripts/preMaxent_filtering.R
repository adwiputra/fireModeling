...
# remove the attribute columns from ignitial and spread_cleanTable
covVariables <- names(attribute_allPoints_withCov_withoutNA) %>% setdiff(c(names(attribute_allPoints), "LONgrd", "LATgrd", "TRACK"))
ignitial_cleanTable <- ignitial_cleanTable %>% select(all_of(covVariables))
spread_cleanTable <- spread_cleanTable %>% select(all_of(covVariables))
# the _cleanTables can now be input into maxent run
# export as csv
ignitial_cleanTable %>% write.csv("data/ignitial_cleanTable.csv", row.names = FALSE)
spread_cleanTable %>% write.csv("data/spread_cleanTable.csv", row.names = FALSE)
