...
responseCurves_compile_nTree500 <- fread(paste0(inputDir, "medianResponseCurves_ignitionPercent_100_nTree500_reclassed.csv"))
landCover_lookup <- read.csv("E:/temp_transfer/finalized_materials/landCover_lookup_v3.csv")
ecoregion_lookup <- read.csv("E:/temp_transfer/finalized_materials/ecoregion_lookup_v3.csv")
...
# b1. domLandCover
landCover_lookup <- landCover_lookup %>% arrange(v3.val) %>% mutate(v3.val = as.factor(as.character(v3.val)))
lookup_toJoin <- landCover_lookup
...
# b2. ecoregion
ecoregion_lookup <- ecoregion_lookup %>% arrange(v3.val) %>% mutate(v3.val = as.factor(as.character(v3.val)))
lookup_toJoin <- ecoregion_lookup
...
png(file = paste0(inputDir, "plots/jun2025/jun30/figure4_landCovDominant_bar.png"), width=2100, height = 1280, units = "px")
...
dev.off()
# 
png(file = paste0(inputDir, "plots/jun2025/jun30/figure4_landCovDominant.png"), width=2100, height = 1280, units = "px")
...
# 3. PLOTTING=========
# a. Bar plot with standard deviation; adopted with modification from https://r-graph-gallery.com/4-barplot-with-error-bar.html 
png(file = paste0(inputDir, "plots/jun2025/jun30/figure4_ecoregion_bar.png"), width=2100, height = 1350, units = "px")
ggplot(summarized_fireIgnitionResponse) +
...
dev.off()
# 
png(file = paste0(inputDir, "plots/jun2025/jun30/figure4_ecoregion.png"), width=2100, height = 1280, units = "px")
...
# 
# 
# landCoverDir <- "E:/temp_transfer/data/spatial/covariates/landCover_CCI/"
# categoricalVariables <- c("gridcode", "reduced_ecoregion_raster")
# landCover_freq <- "E:/temp_transfer/finalized_materials/na_synced_covariates_v2/dominantLandCover.tif" %>% rast() %>% as.factor() %>% freq()
# # Ecoregion raster attribute table
# ecoregion_freq <- "E:/temp_transfer/finalized_materials/na_synced_covariates_v2/Ecoregion.tif" %>% rast() %>% as.factor() %>% freq()
# # Reduced ecoregion vector layer to extract the attribute table
# ecoregion_lookup <- "E:/temp_transfer/finalized_materials/reduced_ecoregion.shp" %>% vect() %>% values()
# # 3. PREPROCESSING======
# 
