...
# Removing some objects that won't be used here to clear up RAM space
rm(acqTime_lookup, attribute_allPoints, nearTable_allPoints)
majorNearTable_rds <- "D:/Documents/otherOpps/YSSP/projects/analysis/data/spatial/merged_nearTable_onlyCoords.rds" %>% readRDS()
retained_fireGraph_convHull <- vect("D:/Documents/research/projects/nus07_fire/data/processed_2024/convexHull_intersected_water.shp") # a subset of the original 'retained_fireGraph_convHull' in "plot_convHull_network.R"
clipped_permWater <- vect("D:/Documents/research/projects/nus07_fire/data/processed_2024/globWater_permanent_patched_clippedByConvexHulls.shp")
...
# 2. Trace the hotspots' id =======
retained_fireGraph <- values(retained_fireGraph_convHull)
retained_fireGraph <- retained_fireGraph %>% select(id) %>% pull()
waterIntersecting_fireGraph <- fireGraph_decompose[retained_fireGraph]
# identify the point FIDs
waterEdge_points_FID <- waterIntersecting_fireGraph %>% sapply(function(grph) V(grph)[]) %>% unlist %>% names() %>% as.numeric() %>% unique()
# ignitialPoints_filtered %>% unlist() %>% names() %>% as.numeric() %>% unique()
# fireGraph_decompose_ret <- fireGraph_decompose[retained_fireGraph_IDs]
# b. Identify the ignitial points and the spread points
# identify the potential  points in graphs whose convexHull intersect waterBodies
...
# Label majorNearTable_rds based on the possibility of the edge(s) intersecting waterBodies
majorNearTable_rds <- majorNearTable_rds %>% mutate(waterIntersect_BL = case_when((OID_ %in% waterEdge_points_FID) | (OID_near %in% waterEdge_points_FID) ~ 1,
                                                                                  TRUE ~ 0))
gc() # Free up memory
# Exported to be input to TEJO7
# majorNearTable_rds %>% write_rds("D:/Documents/research/projects/nus07_fire/data/processed_2024/merged_nearTable_onlyCoords_labelledWater.rds")


# Export to be input to further processing using ArcGIS Pro
majorNearTable_rds <- majorNearTable_rds %>% filter(waterIntersect_BL == 1)
lonlat <- majorNearTable_rds  %>% select(OBJECTID, LONGITUDE, LATITUDE) %>% rename_at(1:3, ~c("object", "x", "y"))
lonlat <- majorNearTable_rds  %>% select(OBJECTID, LON_near, LAT_near) %>% rename_at(1:3, ~c("object", "x", "y")) %>% 
...
slicedTable <- majorNearTable_rds %>% filter(group_id == r) %>% select(OBJECTID) %>% data.frame()
