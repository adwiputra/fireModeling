...
landCover_raster <- "F:/temp_donotDelete/finalized_materials/na_synced_covariates_v2/dominantLandCover.tif" %>% rast() %>% as.factor()
landCover_freq <- landCover_raster %>% freq()
# Ecoregion raster attribute table
ecoregion_raster <- "F:/temp_donotDelete/finalized_materials/na_synced_covariates_v2/Ecoregion.tif" %>% rast() %>% as.factor()
...
write.csv(landCover_lookup, "F:/temp_donotDelete/finalized_materials/landCover_lookup.csv", row.names = FALSE)
#ADhere
...
# Save landCover_lookup as reference for interpretation later
write.csv(ecoregion_lookup, "F:/temp_donotDelete/finalized_materials/ecoregion_lookup.csv", row.names = FALSE)
# 4. PROCESSING======
...
# Export raster
writeRaster(landCover_raster, "F:/temp_donotDelete/finalized_materials/na_synced_covariates_v2/dominantLandCover_v2.tif")
...
# Export raster
writeRaster(ecoregion_raster, "F:/temp_donotDelete/finalized_materials/na_synced_covariates_v2/Ecoregion_v2.tif")
